...
import msvcrt
import shutil
import urllib.request  # 🌐 UUSI: tarvitaan asset-lataukseen

# 📁 Paths and settings
base_dir = os.path.dirname(__file__)
settings_path = os.path.join(base_dir, "settings.txt")
config_path = os.path.join(base_dir, ".stellar_launcher_config.json")  # Ei enää APPDATA
...
# 📁 Minecraft folders (käytetään aina install_sourcea)
game_dir = install_source
...
if not os.path.exists(config_path):
    ...
    try:
        if not os.path.exists(game_dir):
            print("Game directory not found. Please verify install_source path.")
            exit(1)
        else:
            print("Game directory exists. Skipping copy.")
...
objects_dir = os.path.join(install_source, "assets", "objects")
